...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $Lucky_number1 = $_POST['Lucky_number1'];
    $Name1 = $_POST['Name1'];
    $message1 = $_POST['message1'];

    // Check if Lucky Number is already owned
    $check_sql = "SELECT * FROM user_entry_silver1 WHERE Lucky_number1 = '$Lucky_number1'";
    $check_result = mysqli_query($conn, $check_sql);
    if (mysqli_num_rows($check_result) > 0) {
        echo "<script>alert('Error: Lucky Number is already owned!');window.location.href='silver_raf2.php';</script>";
        exit();
    }

    // Check if maximum entries (10) have been reached
    $count_sql = "SELECT COUNT(*) as count FROM user_entry_silver1  WHERE users_id1 = '$users_id1'";
  ...
  if ($count_row['count'] >= 10) {
    ...
    $sql = "INSERT INTO user_entry_silver1  (users_id1, Lucky_number1, Name1, message1) 
            VALUES ('$users_id1','$Lucky_number1','$Name1','$message1')";
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        session_start();
        $_SESSION['id'] = $id;
        header("Location: silver_raf2.php");
        exit();
    } else {
        echo "<script>alert('Error: " . $sql . "<br>" . $conn->error . "');window.location.href='silver_raf2.php';</script>";
    }
    $conn->close();
}
...
$sql = "SELECT * FROM user_entry_silver1";
...
<table class="table1" >
		...
		<?php
$seq = 1;
while ($user_entry_silver1 = mysqli_fetch_assoc($result)) {
       echo "<tr>";
			
    echo "<td>".$user_entry_silver1['users_id1']."</td>";
 
    echo "<td>".$user_entry_silver1['Name1']."</td>";
       echo "<td>".$user_entry_silver1['Lucky_number1']."</td>";
    echo "<td>".$user_entry_silver1['message1']."</td>";
...
<form action="silver_raf2.php" method="post" id="myForm" style="display: none;">
<section id="name-input-container">
 


<input name="users_id1" type="int" readonly value="<?php echo $user_id;?>"> <br><br>

<input name="Name1" type="text" readonly   value="<?php echo $name;?>"> <br><br>
<input name="Lucky_number1" type="text" placeholder="Enter your Lucky Number"> <br><br>
<textarea name="message1" placeholder="Any Message to the Developer?"></textarea> <br><br>
...
document.getElementById("s-ent").addEventListener("click", function() {
    ...
    if (silverBalance >= 1 && !hasLuckyNumber) {
        ...
        xhr.open("POST", "s1_update_balance.php", true);
...
<section id="winner-history">
      ...
      <table id="winner-history-table"  class="table2"  style="width:100%; ">
        ...
        <?php
        $seq = 1;
        while ($s2_winner = mysqli_fetch_assoc($result)) {
          echo "<tr>";
          echo "<td>" . $s2_winner['users_id_s2'] . "</td>";
          echo "<td>" . $s2_winner['Name_s2'] . "</td>";
          echo "<td>" . $s2_winner['Date_s2'] . "</td>";
          echo "</tr>";
          $seq += 1;
        }
        ?>
