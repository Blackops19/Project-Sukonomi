...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "SELECT User_id, password, Status, login_attempts, lockout_time FROM user WHERE email='$email'";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        if ($row['Status'] == 1) {
            echo "<script>alert('Your account is already active.');window.location.href='login.php';</script>";
        } else {
            // Verify the password using password_verify()
            if (password_verify($password, $row['password'])) {
                $_SESSION['user_id'] = $row['User_id'];
                $status = "UPDATE user SET Status = 1, login_attempts = 0, lockout_time = 0 WHERE User_id = " . $row['User_id'];
                if ($conn->query($status) === TRUE) {
                     echo "<script>alert('login Successfully.');window.location.href='udashboard.php';</script>";
                    exit();
                }
            } else {
                // Increment login attempts
                $login_attempts = $row['login_attempts'] + 1;
                                $update_attempts = "UPDATE user SET login_attempts = $login_attempts WHERE User_id = " . $row['User_id'];
                $conn->query($update_attempts);

                if ($login_attempts >= 25) {
                    // Lock out the user for 24 hours
                    $lockout_time = time() + 86400; // 24 hours
                    $update_lockout = "UPDATE user SET lockout_time = $lockout_time WHERE User_id = " . $row['User_id'];
                    $conn->query($update_lockout);
                    $_SESSION['lockout_time'] = $lockout_time;
                    $time_left = $lockout_time - time();
                    echo "<script>alert(' This is your $login_attempts attempt since last open. Please try again after $time_left seconds.');window.location.href='login.php';</script>";
                    ...
                    echo "<script>alert(' This is your $login_attempts attempt since last open.');window.location.href='login.php';</script>";
                    exit();
                }
            }
        }
