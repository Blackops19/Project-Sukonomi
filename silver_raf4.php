...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $users_id3 = $_POST['users_id3'];
    $Lucky_number3 = $_POST['Lucky_number3'];
    $Name3 = $_POST['Name3'];
    $message3 = $_POST['message3'];

    // Check if Lucky Number is already owned
    $check_sql = "SELECT * FROM user_entry_silver3 WHERE Lucky_number3 = '$Lucky_number3'";
    $check_result = mysqli_query($conn, $check_sql);
    if (mysqli_num_rows($check_result) > 0) {
        echo "<script>alert('Error: Lucky Number is already owned!');window.location.href='silver_raf3.php';</script>";
        exit();
    }

    // Check if maximum entries (10) have been reached
    $count_sql = "SELECT COUNT(*) as count FROM user_entry_silver3  WHERE users_id3 = '$users_id3'";
    ...
    $sql = "INSERT INTO user_entry_silver3  (users_id3, Lucky_number3, Name3, message3) 
            VALUES ('$users_id3','$Lucky_number3','$Name3','$message3')";
    ...
    if ($conn->query($sql) === TRUE) {
        ...
        session_start();
        $_SESSION['id'] = $id;
        header("Location: silver_raf4.php");
        exit();
    } else {
        echo "<script>alert('Error: " . $sql . "<br>" . $conn->error . "');window.location.href='silver_raf3.php';</script>";
    }
    $conn->close();
}
?>
...
$sql = "SELECT * FROM user_entry_silver3";
...
<table class="table1" >
		...
		<?php
$seq = 1;
while ($user_entry3 = mysqli_fetch_assoc($result)) {
       echo "<tr>";
			
    echo "<td>".$user_entry3['users_id3']."</td>";
 
    echo "<td>".$user_entry3['Name3']."</td>";
       echo "<td>".$user_entry3['Lucky_number3']."</td>";
    echo "<td>".$user_entry3['message3']."</td>";
...
<form action="silver_raf4.php" method="post" id="myForm" style="display: none;">
<section id="name-input-container">
 

<input name="users_id3" type="int" readonly value="<?php echo $user_id;?>"> <br><br>

<input name="Name3" type="text" readonly   value="<?php echo $name;?>"> <br><br>
<input name="Lucky_number3" type="text" placeholder="Enter your Lucky Number"> <br><br>
<textarea name="message3" placeholder="Any Message to the Developer?"></textarea> <br><br>
...
document.getElementById("s-ent").addEventListener("click", function() {
    ...
    if (silverBalance >= 1 && !hasLuckyNumber) {
        ...
        xhr.open("POST", "s3_update_balance.php", true);
...
<section id="winner-history">
      ...
      <table id="winner-history-table"  class="table2"  style="width:100%; ">
        ...
        <?php
        $seq = 1;
        while ($s4_winner = mysqli_fetch_assoc($result)) {
          echo "<tr>";
          echo "<td>" . $s4_winner['users_id_s4'] . "</td>";
          echo "<td>" . $s4_winner['Name_s4'] . "</td>";
          echo "<td>" . $s4_winner['Date_s4'] . "</td>";
          echo "</tr>";
          $seq += 1;
        }
        ?>
